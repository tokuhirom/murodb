...
mod tests {
    ...
    #[test]
    fn test_where_case_when_keeps_collation_aware_nested_comparison() {
        let (mut pager, mut catalog, _dir) = setup();
        execute(
            "CREATE TABLE t (id BIGINT PRIMARY KEY, name VARCHAR COLLATE nocase)",
            &mut pager,
        ...
        let result = execute(
            "SELECT id FROM t WHERE CASE WHEN name = 'alice' THEN 1 ELSE 0 END = 1",
            &mut pager,
            &mut catalog,
        )
        .unwrap();
    ...
    #[test]
    fn test_where_simple_case_when_keeps_collation_aware_comparison() {
        let (mut pager, mut catalog, _dir) = setup();
        execute(
            "CREATE TABLE t (id BIGINT PRIMARY KEY, name VARCHAR COLLATE nocase)",
            &mut pager,
        ...
        let result = execute(
            "SELECT id FROM t WHERE CASE name WHEN 'alice' THEN 1 ELSE 0 END = 1",
            &mut pager,
            &mut catalog,
        )
        .unwrap();
